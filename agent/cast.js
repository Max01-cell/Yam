...
// Deterministic per character: the same name always yields the same seed, so a reference
// sheet can be regenerated byte-identically months later without storing anything. Venice
// reproduces an image exactly for a given (model, prompt, seed) — verified, not assumed.
// Derived rather than random because a lost seed would mean a lost character design.
...
export function mergeCast(prev, update) {
  ...
  for (const [rawName, rawCh] of Object.entries(incoming)) {
    ...
    base.characters[key ?? name] = {
      ...
      // The look yam is drawing TOWARD: the fixed appearance text handed to the image
      // model verbatim every time. Separate from canon because canon is written for a
      // reader and this is written for a renderer.
      appearance: clampText(rawCh.appearance, MAX_CANON) || existing?.appearance || '',
...
// The canonical reference image for a character, recorded when it is generated. Like the
// practice count this is written by the system at the moment the artifact exists, never
// claimed — a character cannot assert it has a reference sheet it does not have.
export function recordReference(prev, name, url) {
  const clean = normaliseName(name);
  if (!clean || !url) return prev ?? emptyCast();
  const base = {
    characters: { ...(prev?.characters ?? {}) },
    world: prev?.world ?? null,
    retired: [...(prev?.retired ?? [])],
...
export function formatCast(state) {
  ...
  const lines = chars
    ...
    .map(([name, c]) => {
      ...
      const appearance = c?.appearance ? `\n    appearance (sent verbatim to the image model): ${c.appearance}` : '';
      const open = c?.open_questions?.length ? `\n    open: ${c.open_questions.join(' · ')}` : '';
      const last = c?.last_study ? `\n    last study: ${c.last_study}` : '';
      const ref = c?.reference
        ? `\n    reference sheet: ${c.reference}  (look at this; draw toward it)`
        : `\n    reference sheet: NONE YET — propose venice_generate {character:"${name}", prompt:"..."} to make one`;
      return head + canon + appearance + open + ref + last;
    });
